...
let rec checkLength l intg = match l with x::xs -> (checkLength xs (intg-1)) | [] -> (intg = 0);;       (*checks whether l has length intg*)
...
let addSubst t1 t2 sl = match t1 with
V(var) -> (Q(var,t2))::sl 
| _-> (match t2 with V(var2) -> (Q(var2,t1)::sl));;
